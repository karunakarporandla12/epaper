...
const PdfImageViewer = () => {
  ...
  // caches for rendered pages / thumbnails (useRef to persist)
  const pageImageCache = useRef(new Map()); // key: pageIndex (1-based) -> dataURL
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setAllFiles(mapped);
        // optional: collect availableDates set for ArchiveCalendar
        const dates = new Set(mapped.map((m) => m.dateStr).filter(Boolean));
